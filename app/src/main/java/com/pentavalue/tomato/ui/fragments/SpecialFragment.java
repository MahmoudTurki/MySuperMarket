...
import com.pentavalue.tomato.adapter.SpecialAdapter;
...
public class SpecialFragment extends BaseFragment {
    ...
    private void init(View view) {
        ...
        setupRecyclerView(recyclerView, BuildingStaticData.getSpecialsList());
    }

    private void setupRecyclerView(RecyclerView recyclerView, List<Special> list) {
        recyclerView.setLayoutManager(new GridLayoutManager(getActivity(), 3));
//        recyclerView.addItemDecoration(new DividerItemDecoration());
        setListItems(recyclerView, list);
    }

    private void setListItems(RecyclerView recyclerView, List<Special> list) {
        if (!ValidatorUtils.isEmpty(list)) {
            SpecialAdapter adapter = new SpecialAdapter(getActivity(), list);
